...
public static class ModernFilePicker
{
    [ComImport]
...
$path = [ModernFilePicker]::Pick("Select the source file to copy")
...
export async function pickFileWithDialog() {
  if (process.platform !== "win32") {
    return { path: null, error: "File picker is only available on Windows." };
  }

  const scriptPath = path.join(
    os.tmpdir(),
    `automateoffice-file-picker-${process.pid}-${Date.now()}.ps1`,
  );
